...
using Content.Server._Omu._BSD.MultiBlockSystem.Components;

namespace Content.Server._Omu._BSD.MultiBlockSystem;
...
public sealed partial class MultiBlockSystem : EntitySystem
{
    ...
    private void CheckIntegrity(EntityUid uid0, MultiBlockPartComponent comp, ref AnchorStateChangedEvent args)
    {
        CheckIntegrityAll();
        return;
    }
    ...
    private EntityUid CheckSide(EntityUid uid, int sideNum, string allowedTypes, string[] structureTypesAllowed, float margineOfError)
    {
        ...
        while (foundEntities.MoveNext(out var uidLoop, out var transComp, out var multiblockPartComp))
        {
            ...
            if (Transform(uid).GridUid is null || Transform(uidLoop).GridUid is null) continue;//one of them is not on a grid
